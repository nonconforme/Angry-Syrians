...
//collisions
-(BOOL) handleAssadCollision:(CollisionMoment)moment arbiter:(cpArbiter*)arb space:(cpSpace*)space;
...
- (id) initWithSaved:(BOOL)loadIt
{
	...
	[smgr addCollisionCallbackBetweenType:kAssadCollisionType 
								otherType:kGroundCollisionType 
								   target:self 
								 selector:@selector(handleAssadCollision:arbiter:space:)
...
-(BOOL) aboutToReadShape:(cpShape*)shape shapeId:(long)id
{
	...
	else if (shape->collision_type == kAssadCollisionType)
	{
		Assad *assad = [Assad assadWithGame:self shape:shape];
		[self addChild:assad z:5];
	}
...
- (void) setupEnemies
{
    Assad *assad = [Assad assadWithGame:self];
    Shabiha *shabiha = [Shabiha shabihaWithGame:self];
	
    
    switch (numberOfEnemies) {
        case 1:
            assad.position = ccp(250,23);
            [self addChild:assad z:5];
            _enemiesLeft++;
            break;
     ...
     case 2:
            ...
            assad = [Assad assadWithGame:self];
            assad.position = ccp(415,23);
            [self addChild:assad z:5];
            _enemiesLeft++;
        ...
        case 5:
            ...
            shabiha.position = ccp(200,23);
            [self addChild:shabiha z:5];
            _enemiesLeft++;
            ...
            shabiha = [Shabiha shabihaWithGame:self];
            shabiha.position = ccp(150,23);
            [self addChild:shabiha z:5];
            _enemiesLeft++;
            break;
...
- (void) setupBackground
{
	CCSprite *background = [CCSprite spriteWithFile:@"homsback.png"];
...
- (void) setupSettings 
{
    CCMenuItem *settings = [CCMenuItemImage itemFromNormalImage:@"settings.png"
												 selectedImage:@"settings-sel.png"
														target:self
                                                       selector:@selector(settings:)];
    settings.position = ccp(-185,-140);
	[self addChild:[CCMenu menuWithItems:settings,nil] z:100];
...
-(BOOL) handleAssadCollision:(CollisionMoment)moment arbiter:(cpArbiter*)arb space:(cpSpace*)space
{
	CP_ARBITER_GET_SHAPES(arb, assadShape, otherShape);
	
	//Get a value for "force" generated by collision
	float f = cpvdistsq(assadShape->body->v, otherShape->body->v);
	
	if (f > 600)
	{
		[(Assad*)assadShape->data addDamage:f/600];
	}
...
-(void) enemyKilled
{
	...
	if (_enemiesLeft == 0)
	{
        ...
        NSString *username = [mainDelegate getUsername];
		...
		CCLabelTTF *label = [CCLabelTTF labelWithString:textlabel 
											   ...
											   fontSize:40];
		label.position = ccp(240,170);
        
        NSInteger scoreValue = [mainDelegate getScore]; 
        
            
            NSString *scoreString = @"Score :";
    
            NSString *scoreStringInt = [[NSString alloc] initWithFormat:@"%i",scoreValue];
            
            scoreString = [scoreString stringByAppendingString:scoreStringInt];
            
            CCLabelTTF *score = [CCLabelTTF labelWithString:scoreString
                                                  fontName:@"Marker Felt" 
                                                  fontSize:40];
            score.position = ccp(240,100);
